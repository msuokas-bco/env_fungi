---
title: "Data-analysis ITS amplicons"
format: pdf
editor: visual
mainfont: Aptos
...
```{r reload, size="tiny"}
# Load data from rds and set random seed number
tse <- readRDS("results/TSE.rds")
set.seed(123412)
# Create pH group
```

#### Prevalent features on data
...
barplot1 <- ggplot(genus_long, aes(x=Labc, y=Abundance, fill=Genus)) +
    geom_bar(stat="identity", width=0.8) +
    facet_wrap(~ Veg) + bar_theme
barplot1
```

\newpage

Barplot faceted by type

```{r comp_barplot2, size="tiny", fig.dim=c(6.5,4)}
barplot2 <- ggplot(genus_long, aes(x=Labc, y=Abundance, fill=Genus)) +
    geom_bar(stat="identity", width=0.8) + facet_grid(~ Type) + bar_theme
barplot2
```
...
barplot3 <- ggplot(genus_long, aes(x=Labc, y=Abundance, fill=Genus)) +
...
alpha_table <- data.frame(Sample=colData(tse)$Labc,
...
alphadf <- data.frame(colData(tse)) %>%
    dplyr::select(Labc, Area, Veg, Type, pH,
                  Shannon=shannon_diversity)
# Vegetation
plot_vegetation <- ggplot(alphadf, aes(x=Veg, y=Shannon,
                                     fill=Veg)) + geom_boxplot() +
    common_theme + labs(caption="Vegetation")
# Area
plot_area <- ggplot(alphadf, aes(x=Area, y=Shannon,
                                 fill=Area)) + geom_boxplot() +
    common_theme + labs(caption="Area")
# Soil_type
plot_type <- ggplot(alphadf, aes(x=Type, y=Shannon,
                                      fill=Type)) +
    geom_boxplot() + common_theme + labs(caption="Type")
# pH
...
plot_area + plot_vegetation + plot_type + plot_pH
```

Kruskal-Wallis tests for variables

```{r kruskal_wallis, size="tiny"}
kruskal.test(Shannon ~ Veg, data=alphadf)
kruskal.test(Shannon ~ Area, data=alphadf)
kruskal.test(Shannon ~ Type, data=alphadf)
...
p1 <- plotReducedDim(tse, "MDS_bray", colour_by = "Veg")
...
maaslin3_out <- maaslin3(
    ...
    standardize = FALSE,
    min_prevalence = 0)
saveRDS(maaslin3_out, "results/maaslin3.rds")
```

Genera with significantly different abundance

```{r maaslin_results, size="tiny"}
maaslin3_out <- readRDS("results/maaslin3.rds")
maaslin_summary <- maaslin3_out$fit_data_abundance$results %>% dplyr::select(Genus = feature, coef, stderr, qval_joint) %>%
  filter(qval_joint < 0.05) %>% arrange(desc(coef))
# Summary table
...
In Maaslin3 analysis, control is neutral, effect is acidic. Changes are calculated with formula log(q)\*sign(coeff). Compared to Maaslin2, analysis provides more stringent filtering (80 positives vs 156 positives).
